...
Route::get('/test-upload', function() {
    ...
    return response()->json([
        ...
        'url_base' => $config['url'],
        'test_url' => asset('uploads/test.txt'),
        'uploads_dir_exists' => is_dir($config['root']),
        'uploads_writable' => is_writable(dirname($config['root'])),
        'test_file_path' => $config['root'] . '/test.txt',
        'test_file_exists' => file_exists($config['root'] . '/test.txt'),
        'app_env' => env('APP_ENV'),
    ]);
})->middleware('auth');
// CACHE CLEARER ROUTE - CRITICAL FIX
Route::get('/clear-all-cache', function() {
    try {
        ...
        return response()->json([
            'success' => true,
            'message' => '✅ Cache volledig gecleared!',
            'public_path_now' => public_path(),
            'test_again' => url('/test-upload')
        ]);
    } catch (\Exception $e) {
        return response()->json([
            'error' => $e->getMessage()
