...
class KlantenController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($data['email'])) {
            try {
                ...
                ], function ($message) use ($klant) {
                    $message->to($klant->email, $klant->voornaam . ' ' . $klant->naam)
                           ->subject('Welkom bij Bonami Sportcoaching');
                });
                
                // Update klant to show invitation was sent
                $klant->update(['laatste_uitnodiging' => now()]);
                
                $successMessage = 'Klant aangemaakt en uitnodiging verstuurd naar ' . $klant->email;
    ...
    public function sendInvitation(Request $request, Klant $klant)
    {
        try {
            ...
            return redirect()->back()->with('success', 'Uitnodiging verstuurd naar ' . $klant->naam);
