<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('klanten.index')" :active="request()->routeIs('klanten.index')">
                {{ __('Klanten') }}
            </x-responsive-nav-link>
            @if(Auth::user() && Auth::user()->role === 'admin')
                <x-responsive-nav-link :href="route('medewerkers.index')" :active="request()->routeIs('medewerkers.index')">
                    {{ __('Medewerkers') }}
                </x-responsive-nav-link>
            @endif
        </div>
