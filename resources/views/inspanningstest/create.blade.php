...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateProtocolFields() {
        ...
        if (selectedType === 'veldtest_lopen') {
            ...
            document.getElementByld('startwattage').value = 100;
...
// 🤖 COMPLETE AI ANALYSE FUNCTIE (HERSTELD)
async function generateCompleteAIAnalysis() {
    console.log('🤖 generateCompleteAIAnalysis() aangeroepen');
    
    const btn = document.getElementById('ai-complete-btn');
    const textarea = document.getElementById('complete_ai_analyse');
    
    if (!btn || !textarea) {
        console.error('❌ Button of textarea niet gevonden');
        alert('Fout: AI analyse elementen niet gevonden');
        return;
    }
    
    // Disable button tijdens verwerking
    btn.disabled = true;
    btn.innerHTML = '⏳ Bezig met analyseren...';
    btn.classList.add('opacity-50', 'cursor-not-allowed');
    
    try {
        // Verzamel alle form data
        const formData = new FormData(document.getElementById('inspanningstest-form'));
        ...
        console.log('📊 Verzonden data naar AI:', data);
        
        // Roep backend aan voor AI analyse
        const response = await fetch('/api/inspanningstest/generate-complete-ai-analysis', {
            method: 'POST',
            headers: {
        ...
        if (result.success && result.analysis) {
            ...
            btn.innerHTML = '✅ Analyse Compleet!';
            ...
            throw new Error(result.message || 'Geen analyse ontvangen');
        }
        
    } catch (error) {
        console.error('❌ AI analyse fout:', error);
        alert('Fout bij genereren AI analyse: ' + error.message);
...
// 🎯 TRAININGSZONES FUNCTIE (HERSTELD)
function updateTrainingszones() {
    console.log('🎯 updateTrainingszones() aangeroepen');
    ...
    if (!methodeSelektor || !aantalSelektor || !eenheidSelektor || !container) {
        console.error('❌ Trainingszones elementen niet gevonden');
        return;
    }
    
    const methode = methodeSelektor.value;
    const aantal = parseInt(aantalSelektor.value) || 6;
    const eenheid = eenheidSelektor.value;
    
    console.log('📊 Zones configuratie:', { methode, aantal, eenheid });
    
    if (!methode) {
        container.style.display = 'none';
        console.log('⏭️ Geen methode geselecteerd - zones verborgen');
        return;
    }
    
    // Haal drempelwaarden op
    const LT1_vermogen = parseFloat(document.getElementById('aerobe_drempel_vermogen')?.value) || 0;
    const LT2_vermogen = parseFloat(document.getElementById('anaerobe_drempel_vermogen')?.value) || 0;
    const LT1_hartslag = parseInt(document.getElementById('aerobe_drempel_hartslag')?.value) || 0;
    const LT2_hartslag = parseInt(document.getElementById('anaerobe_drempel_hartslag')?.value) || 0;
    
    console.log('🎯 Drempelwaarden:', { LT1_vermogen, LT2_vermogen, LT1_hartslag, LT2_hartslag });
    
    if (LT1_hartslag === 0 || LT2_hartslag === 0) {
        alert('⚠️ Vul eerst de aerobe en anaerobe drempelwaarden in voordat je zones berekent.');
        container.style.display = 'none';
        return;
    }
    
    // Bereken zones op basis van methode (simplified example - implement full logic)
    const zones = berekenBonamiZones(LT1_hartslag, LT2_hartslag, aantal);
    
    // Toon zones tabel
    toonZonesTabel(zones, eenheid);
    container.style.display = 'block';
    
    console.log('✅ Trainingszones berekend en getoond');
}

// Helper functie: Bereken Bonami zones
function berekenBonamiZones(LT1_HR, LT2_HR, aantal) {
    const zones = [];
    
    // Simpele 6-zones Bonami methode
    const zone1_max = Math.round(LT1_HR * 0.85);
    const zone2_max = Math.round(LT1_HR * 0.95);
    const zone3_max = Math.round(LT1_HR);
    const zone4_max = Math.round((LT1_HR + LT2_HR) / 2);
    const zone5_max = Math.round(LT2_HR);
    const zone6_max = Math.round(LT2_HR * 1.10);
    
    zones.push({ naam: 'Zone 1 - Herstel', min: 0, max: zone1_max });
    zones.push({ naam: 'Zone 2 - Basis', min: zone1_max + 1, max: zone2_max });
    zones.push({ naam: 'Zone 3 - Tempo', min: zone2_max + 1, max: zone3_max });
    zones.push({ naam: 'Zone 4 - Drempel', min: zone3_max + 1, max: zone4_max });
    zones.push({ naam: 'Zone 5 - VO2max', min: zone4_max + 1, max: zone5_max });
    zones.push({ naam: 'Zone 6 - Anaeroob', min: zone5_max + 1, max: zone6_max });
    
    return zones.slice(0, aantal);
}

// Helper functie: Toon zones tabel
function toonZonesTabel(zones, eenheid) {
    ...
    header.innerHTML = `
        <tr>
            ...
            <th class="px-4 py-2 text-left">Min ${eenheid === 'hartslag' ? '(bpm)' : ''}</th>
            <th class="px-4 py-2 text-left">Max ${eenheid === 'hartslag' ? '(bpm)' : ''}</th>
        </tr>
    `;
    
    // Body
    tbody.innerHTML = zones.map((zone, idx) => `
        <tr class="hover:bg-gray-50">
            <td class="px-4 py-2 border">Zone ${idx + 1}</td>
            <td class="px-4 py-2 border">${zone.naam}</td>
            <td class="px-4 py-2 border">${zone.min}</td>
            <td class="px-4 py-2 border">${zone.max}</td>
        </tr>
    `).join('');
    
    // Bewaar zones data in hidden input
    document.getElementById('trainingszones_data').value = JSON.stringify(zones);
}
</script>
@endsection
