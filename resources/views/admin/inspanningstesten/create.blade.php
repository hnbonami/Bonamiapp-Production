    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">✨ Nieuwe Trainingszones Template</h1>
                    <p class="mt-2 text-gray-600">Maak een custom zones configuratie voor jouw organisatie</p>
                </div>
            </div>
        </div>

        <form action="{{ route('admin.inspanningstesten.store') }}" method="POST" id="zone-template-form">
            @csrf
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <select name="sport_type" 
                                        ...
                                        required
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                    <option value="beide" {{ old('sport_type') == 'beide' ? 'selected' : '' }}>🏆 Beide (Fietsen & Lopen)</option>
                                    <option value="fietsen" {{ old('sport_type') == 'fietsen' ? 'selected' : '' }}>🚴 Fietsen</option>
                                    <option value="lopen" {{ old('sport_type') == 'lopen' ? 'selected' : '' }}>🏃 Lopen</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="berekening_basis" 
                                        ...
                                        required
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                    <option value="lt2" {{ old('berekening_basis') == 'lt2' ? 'selected' : '' }}>LT2 (Anaërobe Drempel)</option>
                                    <option value="lt1" {{ old('berekening_basis') == 'lt1' ? 'selected' : '' }}>LT1 (Aërobe Drempel)</option>
                                    <option value="max" {{ old('berekening_basis') == 'max' ? 'selected' : '' }}>MAX (Maximaal Vermogen)</option>
                                    <option value="ftp" {{ old('berekening_basis') == 'ftp' ? 'selected' : '' }}>FTP (Functional Threshold Power)</option>
                                    <option value="custom" {{ old('berekening_basis') == 'custom' ? 'selected' : '' }}>Custom</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="beschrijving" 
                                          ...
                                          rows="3"
                                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                          placeholder="Beschrijf wanneer deze template gebruikt moet worden...">{{ old('beschrijving') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-md p-6">
                        ...
                        <div id="zones-container" class="space-y-4">
                            <!-- Zones worden hier dynamisch toegevoegd -->
                        </div>
...
// Voeg initiële zone toe bij laden
document.addEventListener('DOMContentLoaded', function() {
    addZone(); // Start met 1 zone
