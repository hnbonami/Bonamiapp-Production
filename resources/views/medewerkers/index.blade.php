...
@section('content')
<!-- Topbar trimmed: actions moved under the stats tile -->
<h2 class="text-2xl font-bold mb-1">Medewerkerslijst</h2>
<div style="background:#fff;border-radius:18px;box-shadow:0 2px 8px #e5e7eb;padding:1.1em 1.2em 0.7em 1.2em;display:flex;flex-direction:column;align-items:flex-start;min-width:220px;max-width:300px;margin-bottom:2em;">
    <div style="display:flex;align-items:center;gap:0.5em;">
    <span style="background:#fef3e2;border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;aspect-ratio:1/1;">
...
<!-- Actions moved under stats tile -->
<div style="display:flex;gap:0.7em;align-items:center;margin:1.2em 0;">
    ...
    <a href="{{ route('medewerkers.export') }}" aria-label="Export Excel" title="Export Excel" class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-emerald-100 text-emerald-800" style="margin-right:0.2rem;text-decoration:none;">
    ...
    <form method="GET" action="" id="zoekForm" style="display:inline-flex;align-items:center;gap:0.3em;margin-left:auto;">
        <input type="text" name="zoek" id="zoekInput" value="{{ request('zoek') }}" placeholder="Zoek medewerker..." style="padding:0.25em 0.9em;border:1.2px solid #d1d5db;border-radius:7px;font-size:0.95em;width:160px;box-shadow:0 1px 3px #f3f4f6;" />
    </form>
</div>
            ...
            <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">E-mail</th>
                ...
                <th class="px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider"></th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-gray-700">{{ $medewerker->voornaam }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-700">{{ $medewerker->email }}</td>
                            <td class="px-6 py-4">
                                <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold {{ $medewerker->status === 'Actief' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                                    {{ $medewerker->status }}
                                </span>
                            </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right align-top">
                                    ...
                                    <a href="{{ route('medewerkers.edit', $medewerker->id) }}" aria-label="Bewerk" class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-orange-100 text-orange-800" title="Bewerk">
                                    ...
                                    <a href="{{ route('medewerkers.show', $medewerker->id) }}" aria-label="Profiel" class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-blue-800" title="Profiel">
                                    ...
                                    <form action="{{ route('medewerkers.invite', $medewerker) }}" method="POST" class="inline" onsubmit="return confirm('Uitnodiging versturen naar {{ $medewerker->email }}?')">
                                        @csrf
                                    ...
                                    <form action="{{ route('medewerkers.destroy', $medewerker->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
