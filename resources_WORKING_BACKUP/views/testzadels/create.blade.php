...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Nieuwe Testzadel Uitlening</h1>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="type" id="type" 
                                ...
                                <option value="Comfort" {{ old('type') == 'Comfort' ? 'selected' : '' }}>Comfort</option>
                                <option value="Sport" {{ old('type') == 'Sport' ? 'selected' : '' }}>Sport</option>
                                <option value="Race" {{ old('type') == 'Race' ? 'selected' : '' }}>Race</option>
                                <option value="Endurance" {{ old('type') == 'Endurance' ? 'selected' : '' }}>Endurance</option>
                                <option value="MTB" {{ old('type') == 'MTB' ? 'selected' : '' }}>MTB</option>
                                <option value="Gravel" {{ old('type') == 'Gravel' ? 'selected' : '' }}>Gravel</option>
                            </select>
                        </div>

                        <div>
                            <label for="breedte" class="block text-sm font-medium text-gray-700 mb-2">
                                Breedte (mm) <span class="text-red-500">*</span>
                            </label>
                            <select name="breedte" id="breedte" required
                                    class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                <option value="">Selecteer breedte...</option>
                                <option value="130" {{ old('breedte') == '130' ? 'selected' : '' }}>130mm</option>
                                <option value="143" {{ old('breedte') == '143' ? 'selected' : '' }}>143mm</option>
                                <option value="155" {{ old('breedte') == '155' ? 'selected' : '' }}>155mm</option>
                                <option value="165" {{ old('breedte') == '165' ? 'selected' : '' }}>165mm</option>
                            </select>
                            @error('breedte')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
