...
<body>
    ...
    <div id="mobile-nav" class="md:hidden hidden fixed top-14 left-0 right-0 z-30 flex-col gap-2 pb-3 bg-gray-50 border-b border-gray-200" style="padding-left:2em;padding-right:1rem;">
        ...
        @endif
        @if(Auth::user() && Auth::user()->role !== 'klant')
            <a href="/report-templates" class="flex items-center justify-between font-semibold text-base py-2 border-b-2 {{ str_starts_with($routeName, 'report_templates') ? 'border-[#c8e1eb] text-black' : 'border-transparent text-gray-900' }} transition-all duration-200 hover:text-black">Sjablonen <span class="ml-1 inline-flex items-center justify-center px-2 py-0.5 text-xs font-medium bg-[#c1dfeb] text-[#08474f] rounded-full">{{ \App\Models\ReportTemplate::count() }}</span></a>
        @endif
    </div>
    </div>
    ...
    <aside class="hidden md:flex md:flex-col bg-white border-r border-gray-200 fixed left-0 right-auto z-50 overflow-y-auto pointer-events-auto" style="top:56px; bottom:0; width:240px;">
            ...
            @endif
                ...
                <a href="/templates" class="relative flex items-center gap-3 pl-24 pr-3 py-2 transition-colors {{ request()->is('templates*') ? 'text-gray-900' : 'text-gray-700 hover:text-gray-900' }}" style="padding-left:48px;{{ request()->is('templates*') ? 'background:#f6fbfe' : '' }}">
