...
<div class="max-w-4xl pt-0 pb-6">
    ...
    <div class="flex items-start mb-6" style="gap:3.75rem;">
        <div class="flex flex-col items-start gap-2">
            ...
            @endif
            <form method="POST" action="{{ route('klanten.avatar', $klant->id) }}" enctype="multipart/form-data">
                @csrf
                <input type="file" name="avatar" id="avatarKlantInline" accept="image/*" style="display:none;">
                <div class="flex items-center gap-2">
                    <button type="button" id="avatarCamBtnKlantInline" aria-label="Maak foto" title="Maak foto" class="bg-gray-100 text-gray-800 rounded-full" style="width:36px;height:36px;display:inline-flex;align-items:center;justify-content:center;box-shadow:0 1px 3px #e0e7ff;">
                    ...
                    <button type="button" id="avatarBtnKlantInline" class="rounded-full px-3 py-1 bg-gray-100 text-gray-800 font-semibold text-xs">Wijzig foto</button>
                </div>
                <button type="submit" id="avatarSubmitKlantInline" style="display:none;" class="rounded-full px-3 py-1 bg-indigo-100 text-indigo-800 font-semibold text-xs">Opslaan</button>
            </form>
            <script>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right align-top">
                            <div class="action-buttons flex flex-row flex-nowrap items-center justify-end gap-2">
                                <a href="{{ route('bikefit.report.print.perfect', ['klant' => $klant->id, 'bikefit' => $test->id]) }}" aria-label="Preview" class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-blue-800" title="Preview">
