    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold">Notitie bewerken</h2>
            ...
            <form method="POST" action="{{ route('staff-notes.update', $staffNote) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" 
                           name="title" 
                           id="title" 
                           value="{{ old('title', $staffNote->title) }}"
                ...
                <div class="mb-4">
                    ...
                    <select name="visibility" 
                            ...
                            required>
                        <option value="staff" {{ old('visibility', $staffNote->visibility) == 'staff' ? 'selected' : '' }}>
                            🏢 Alleen medewerkers & admin
                        </option>
                        <option value="all" {{ old('visibility', $staffNote->visibility) == 'all' ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="content" 
                              ...
                              required>{{ old('content', $staffNote->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
...
ClassicEditor
    .create(document.querySelector('#content'), {
        ...
        simpleUpload: {
            uploadUrl: '{{ route("tinymce.upload") }}',
            headers: {
    ...
    .then(newEditor => {
        ...
        // Handle form submission - beter event handling
        ...
        // Backup: also handle form submit event
